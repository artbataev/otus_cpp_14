...
class AssignmentTestFromFileNoDuplicates : public testing::TestWithParam<TestParams> {
};

TEST_P(AssignmentTestFromFileNoDuplicates, AssignmentExampleOptimized) {
    auto task_runner = mapreduce::MapReduceRunner<
...
INSTANTIATE_TEST_CASE_P(MyGroup, AssignmentTestFromFile, ::testing::Values(
        ...
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.mg-no-new-line.in.txt"s, 1, 1, 3},
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.mg-no-new-line.in.txt"s, 2, 2, 3},
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.mg-no-new-line.in.txt"s, 3, 3, 3}
));


// only tests without duplicated emails
INSTANTIATE_TEST_CASE_P(MyGroup, AssignmentTestFromFileNoDuplicates, ::testing::Values(
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.1.in.txt"s, 1, 1, 8},
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.1.in.txt"s, 2, 1, 8},
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.1.in.txt"s, 1, 2, 8},
        TestParams{PROJECT_SOURCE_DIR + "/test/data/test.1.in.txt"s, 4, 4, 8},
